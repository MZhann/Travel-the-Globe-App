...
import { searchUsers, getFeaturedUsers } from '../api/users';
...
export default function ExploreUsers({ onUserSelect, onClose }: ExploreUsersProps) {
  ...
  useEffect(() => {
    const loadFeatured = async () => {
      try {
        const res = await getFeaturedUsers(12);
        setFeaturedUsers(res.users);
      } catch (err) {
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Discover globetrotters and get inspired by their journeys
            </p>
          </div>
          <button
            type="button"
          ...
          {activeTab === 'featured' && (
            <div>
              <h2 className="mb-6 text-xl font-semibold text-white">
                Most Traveled Adventurers
              </h2>
              {featuredUsers.length === 0 ? (
                <div className="flex items-center justify-center py-12">
                  <p className="text-slate-400">Loading featured travelers...</p>
                </div>
              ) : (
...
function UserCard({ user, onClick }: UserCardProps) {
  const percentage = 195 > 0 ? Math.round((user.visitedCount / 195) * 100) : 0;
      ...
      <div className="relative">
        ...
        <div className="mb-4 grid grid-cols-3 gap-3">
          <div className="text-center">
            <div className="text-2xl font-bold text-green-400">{user.visitedCount}</div>
            <div className="text-xs text-slate-500">Visited</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-400">{user.wishlistCount}</div>
        ...
        <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
          <div
            className="h-full rounded-full bg-gradient-to-r from-green-500 to-amber-500 transition-all"
        ...
        <div className="mt-4 flex items-center justify-between text-xs">
          ...
          {user.albumsPublic ? (
            ...
            <span className="rounded-full bg-slate-700/50 px-2 py-1 text-slate-500">
              🔒 Private
            </span>
          )}
        </div>
